...
\begin{abstract}
County-level farmland consolidation is difficult for deep reinforcement learning because parcel simulation is slow and learned rewards can be exploited. We propose a model-based geocomputational framework that replaces parcel simulation during training with a lightweight learned environment, while evaluating final policies in the real county environment. A 237K-parameter transition model learns from 12{,}000 real-environment transitions, and an observational treatment-effect-informed factor ($\alpha=0.185$) rescales learned rewards. Across 13 townships, 2{,}600 consolidation blocks, and 52{,}515 parcels, uncalibrated model-based policies achieved $-0.976\% \pm 0.129\%$ slope change. Calibration improved performance to $-1.102\% \pm 0.100\%$, a 13.0\% gain over the uncalibrated learned environment ($p=0.004$). The pre-specified factor ranked second among eight reward scales, within 3.1\% of the grid-search optimum and 22.1\% stronger than no scaling. The CPU pipeline runs in about 2 hours, compared with 8--12 GPU-hours for model-free baselines. Recorded-action and 15-seed policy-induced learned-vs-real diagnostics bounded surrogate error: policy-induced rollouts retained 0.998 action-mask agreement, mean support distance of 0.013, and $-1.106\%$ real-environment slope change. Reward errors remained non-negligible, so learned rollouts are used only for training and diagnostics, not final outcome reporting. Propensity-score sensitivity checks frame calibration as observational regularization rather than definitive causal identification. On Dongxing District, a slope-only RL-lite learned block-preference policy achieved $-1.996\%$ slope change; this is external actionability evidence, not full cross-county learned-policy transfer. An executable audit verifies the stored Bishan and bounded Dongxing evidence chains.
\end{abstract}
...
To probe external validity beyond the Bishan training case, we additionally apply the geospatial preprocessing, public-DEM slope enrichment, block action-space construction, non-RL paired-swap dynamics, and a slope-only RL-lite learned block-preference policy to Dongxing District, Neijiang. This check is reported as external slope-only data/actionability evidence, not as full cross-county learned-policy performance under the Bishan multi-objective reward.
...
Dongxing external check & Data audit, DEM slopes, blocks, non-RL dynamics, slope-only RL-lite & External slope-only actionability, not full policy transfer \\
\bottomrule
\end{tabular}
\end{table}
...
\begin{table}[htbp]
\centering
\caption{External-county preprocessing validation on Dongxing District. The table reports data-ingestion and action-space construction results; dynamic and slope-only learned-policy checks are reported in Tables~\ref{tab:dongxing_dynamic} and~\ref{tab:dongxing_rl_lite}.}
...
To test whether Dongxing supports learned sequential block selection under this external slope-only environment, we added a lightweight RL-lite experiment. The environment exposes the 2{,}978 Dongxing blocks as masked actions, uses the same paired-slope-swap dynamics, and rewards reductions in county-wide area-weighted farmland slope. A tabular preference learner updated linear block-feature weights from interactive slope-improvement rewards over 5 training seeds and 200 episodes per seed. The learned policy was then evaluated over 5 deterministic seeds and compared with the dynamic non-RL baselines (Table~\ref{tab:dongxing_rl_lite}). The learned preference policy reduced slope by 1.996\%, exceeding random dynamic episodes by 1.563 percentage points, dynamic slope-gap by 0.778 points, and dynamic area-weighted gap by 1.113 points. This adds external learned-actionability evidence, but it is still a slope-only Dongxing experiment. It does not test transfer of the Bishan transition model, the Bishan policy, or the full Bishan reward with contiguity and \textit{baimu fang} objectives.

\begin{table}[htbp]
\centering
\scriptsize
\setlength{\tabcolsep}{3pt}
\caption{Dongxing slope-only RL-lite learned-policy experiment. The learned policy is a tabular preference learner trained on Dongxing slope-swap interactions; all methods use 100 decision steps and at most five positive-gap parcel pairs per selected block. This table reports external slope-only actionability, not full cross-county policy transfer.}
...
\textbf{Cross-county performance.} Full policy-performance experiments remain limited to Bishan District. The Dongxing experiment shows that the cadastral preprocessing, DEM-based slope enrichment, block action-space construction, static slope-opportunity screening, dynamic non-RL paired-swap baselines, and a slope-only RL-lite learned preference policy run on a second county-scale dataset. It does not test whether a Bishan-trained transition model or Bishan-trained policy transfers to Dongxing. The Dongxing learned-policy result is slope-only and does not include the full Bishan reward components for contiguity and \textit{baimu fang}. Full cross-county performance claims require Dongxing trajectories, local transition-model training or transfer testing, local reward calibration, and learned-policy evaluation under the full multi-objective environment.
...
\item \textbf{External-county slope-only actionability is supported, but full policy transfer remains open.} On Dongxing District, the pipeline constructs 2{,}978 mixed farmland-forest blocks from 134{,}369 parcels. Dynamic non-RL paired-swap baselines reduce area-weighted farmland slope by 0.883--1.218\%, and a slope-only RL-lite learned preference policy reaches $-1.996\%$. These results support external data-pipeline, action-space, and slope-only learned-actionability portability, not cross-county transfer of the Bishan learned environment or full multi-objective policy.
...
\item \textbf{The verified claim boundary is narrower than a full cross-county RL claim.} The end-to-end audit supports the Bishan learned-policy chain and classifies Dongxing as external slope-only actionability. A stronger external-validity claim would require Dongxing trajectories, local transition-model training or transfer testing, reward calibration, learned-policy optimization, and full multi-objective real-environment evaluation on Dongxing.
\end{enumerate}
