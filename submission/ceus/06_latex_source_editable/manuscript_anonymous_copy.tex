...
\begin{abstract}
County-level farmland consolidation is difficult for deep reinforcement learning because parcel-level simulation is slow and learned policies can exploit noisy rewards. We propose a model-based geocomputational framework that replaces parcel simulation during training with a lightweight learned environment, while evaluating final policies in the real county environment. A 237K-parameter transition model learns dynamics from 12{,}000 real-environment transitions, and an observational treatment-effect-informed calibration factor ($\alpha=0.185$) rescales learned rewards to reduce exploitation. Across 13 townships, 2{,}600 consolidation blocks, and 52{,}515 parcels, uncalibrated model-based policies achieved $-0.976\% \pm 0.129\%$ slope change. Calibration improved performance to $-1.102\% \pm 0.100\%$, a 13.0\% gain over the uncalibrated learned environment ($p=0.004$), and the pre-specified factor was within 3.1\% of the grid-search optimum. The CPU pipeline runs in about 2 hours, compared with 8--12 GPU-hours for model-free baselines. Recorded-action and 15-seed policy-induced learned-vs-real diagnostics bound the surrogate error: policy-induced rollouts retained 0.998 action-mask agreement, mean support distance of 0.013, and $-1.106\%$ real-environment slope change. Reward errors remained non-negligible, so learned rollouts are used only for training and diagnostics, not final outcome reporting. Propensity-score sensitivity checks frame calibration as observational regularization rather than definitive causal identification. A Dongxing external-county preprocessing and dynamic-baseline check supports data/action-space feasibility, but not learned-policy transfer. An executable audit verifies the stored Bishan evidence chain and these claim boundaries.
\end{abstract}
