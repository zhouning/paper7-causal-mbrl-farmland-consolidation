...
\begin{abstract}
Deep reinforcement learning (DRL) for spatial planning can be limited by expensive environment interactions: county-level farmland consolidation optimization requires 8--12 hours on A100 GPUs per training run, and some runs fail because policies drift toward reward-model exploitation. We propose a model-based geocomputational framework that replaces parcel-level simulation with a lightweight learned environment for reinforcement-learning-based farmland consolidation planning. A 237K-parameter transition model learns state dynamics from 12{,}000 trajectories collected under random and greedy behavioural policies and supports policy training on CPU. To reduce reward exploitation in the learned environment, we introduce observational treatment-effect-informed reward calibration, which estimates action-quality reward differentials from real trajectory data and uses them to pre-specify a reward-scaling factor. Experiments across 13 townships, 2{,}600 consolidation blocks and 52{,}515 parcels show that uncalibrated model-based policies achieve $-0.976\% \pm 0.129\%$ slope reduction on the real environment; GPU-trained model-free baselines evaluated under the same county-level task achieved $-0.84\%$ and $-0.79\%$. Calibration improves model-based performance to $-1.102\% \pm 0.100\%$, a statistically significant 13.0\% gain over the uncalibrated learned environment ($p = 0.004$), and the pre-specified calibration factor ($\alpha = 0.185$) is within 3\% of the grid-search optimum. Stress tests show that aggregate transition cosine similarity can be misleading: recorded-action 100-step rollouts retain 0.997 action-mask agreement but show increasing global-state and reward errors. A stricter synchronized learned-vs-real diagnostic using trained calibrated policies shows similar mask agreement (0.997), small nearest-neighbour distance to trajectory support (0.011 in normalized global-feature space), and $-1.109\%$ mean real-environment slope change, while confirming that rewards remain noisy and final outcomes must be evaluated in the real environment. Propensity-score diagnostics further indicate that the calibration signal should be read as observational regularization rather than strong causal identification. As an external-county check, the same preprocessing pipeline constructs 2{,}978 mixed farmland-forest blocks from 134{,}369 Dongxing District parcels, and non-RL dynamic paired-swap baselines reduce area-weighted farmland slope by 0.883--1.218\%, compared with 0.358\% for random dynamic selection. An end-to-end evidence audit verifies the Bishan data-to-result chain and explicitly bounds Dongxing as external feasibility evidence rather than learned-policy transfer.
\end{abstract}
...
To probe external validity beyond the Bishan training case, we additionally apply the geospatial preprocessing, public-DEM slope enrichment, block action-space construction, and non-RL paired-swap dynamics to Dongxing District, Neijiang. This check is reported as external data, action-space, and dynamic feasibility evidence, not as cross-county learned-policy performance.
...
\centering
Real county environment $\rightarrow$ random and greedy trajectory collection $\rightarrow$ neural transition model $\rightarrow$ learned county environment $\rightarrow$ policy training with optional observational reward calibration $\rightarrow$ deterministic evaluation on the real county environment.
\end{minipage}}
...
\textbf{End-to-end evidence audit.} To make the verification boundary explicit, we added an executable audit script, \path{paper7/end_to_end_validation.py}, which reads the recorded assets and result files rather than retraining the expensive RL models. The audit checks the six trajectory assets, transition-model checkpoint and training history, 15 paired calibrated/uncalibrated real-environment evaluation seeds, the 40-run reward-scaling grid, recorded-action rollout diagnostics, policy-induced learned-vs-real diagnostics, calibration sensitivity diagnostics, Bishan non-learning baselines, and Dongxing data/action-space/dynamic-baseline outputs. Its output, \path{paper7/results/revision/end_to_end_validation.json}, classifies each manuscript claim by evidence level (Table~\ref{tab:e2e_scope}). This audit verifies consistency of the data-to-result chain used in the paper; it does not replace full retraining when computational budgets allow.
...
Dongxing external check & Data audit, DEM slopes, blocks, non-RL dynamics & Feasibility, not policy transfer \\
\bottomrule
\end{tabular}
\end{table}
...
To avoid relying on aggregate cosine similarity alone, we also rolled the learned transition model forward over recorded real-environment action sequences. Diagnostics were computed at horizons 1, 5, 10, 25, 50, and 100 over 120 deterministic start states. Table~\ref{tab:rollout_diag} reports selected horizons. The selected-block and reward errors are more informative than all-block MAE because only one selected block changes at each step.

\begin{table}[htbp]
\centering
\scriptsize
\setlength{\tabcolsep}{4pt}
\caption{Multi-step transition-model rollout diagnostics over recorded real-environment actions. Selected-block MAE is computed for the acted-on block; global MAE is over 12 county-level features; reward MAE is absolute reward error; action-mask agreement compares valid-block masks derived from predicted and true block features.}
\label{tab:rollout_diag}
\begin{tabular}{lccccc}
\toprule
Horizon & Steps eval. & Sel. block MAE & Global MAE & Reward MAE & Mask agreement \\
\midrule
1   & 120    & 0.093 & 0.011 & 0.136 & 0.99995 \\
10  & 1{,}200 & 0.080 & 0.019 & 0.198 & 0.99960 \\
50  & 6{,}000 & 0.076 & 0.038 & 0.230 & 0.99866 \\
100 & 12{,}000 & 0.075 & 0.049 & 0.234 & 0.99738 \\
\bottomrule
\end{tabular}
\end{table}

The rollout diagnostics support using the learned environment for policy optimization but also reveal its limits. Action-mask agreement remains high even at 100 steps (0.997), indicating that the learned environment usually preserves which blocks remain selectable. Global-state and reward errors increase with rollout horizon, however, so the learned environment should be viewed as an approximate training surrogate whose policies require final real-environment evaluation.

The recorded-action rollouts above test the transition model on behavioural action sequences. To test policy-induced states, we next replayed actions selected by three trained calibrated policies in synchronized learned and real environments. Table~\ref{tab:policy_induced_diag} reports the episode-level means. The trained policies did not move the real environment far from the recorded global-state support: the mean nearest-neighbour distance to the 12{,}000-transition trajectory set was 0.011, with a 95th percentile of 0.016. Action-mask agreement remained 0.997, close to the recorded-action 100-step value. The final real-environment slope change across these three diagnostic policies averaged $-1.109\%$, consistent with the full 15-seed calibrated evaluation ($-1.102\%$). Reward error was larger under policy-induced actions than under recorded actions (raw reward MAE 0.602), but the reward signal actually used during calibrated training had a lower scaled error (0.111). These results strengthen the surrogate-training claim while preserving the paper's central boundary: learned-environment rollouts are diagnostic evidence, whereas final planning outcomes are still reported from the real environment.

\begin{table}[htbp]
\centering
\scriptsize
\setlength{\tabcolsep}{4pt}
\caption{Policy-induced learned-vs-real diagnostics for three calibrated policy checkpoints. The policy selects actions from learned-environment states; the same actions are replayed in the real environment. Support distance is Euclidean nearest-neighbour distance from the real global state to the 12{,}000-transition trajectory support. Calibrated reward MAE reports the same reward error after multiplying by $\alpha=0.185$, the reward scale used during policy training.}
\label{tab:policy_induced_diag}
\begin{tabular}{lcc}
\toprule
Metric & Mean & Range across policies \\
\midrule
Episodes / steps per policy & 3 / 100 & --- \\
Selected-block MAE & 0.075 & 0.074--0.076 \\
All-block MAE & 0.0027 & 0.0026--0.0028 \\
Global-feature MAE & 0.052 & 0.051--0.053 \\
Raw reward MAE & 0.602 & 0.592--0.619 \\
Calibrated reward MAE & 0.111 & 0.109--0.114 \\
Action-mask agreement & 0.997 & 0.997--0.998 \\
Support distance & 0.011 & 0.010--0.012 \\
Support distance, 95th percentile & 0.016 & 0.014--0.019 \\
Final real slope change (\%) & $-1.109$ & $-1.180$--$-0.999$ \\
\bottomrule
\end{tabular}
\end{table}
...
\subsection{External-county data and action-space validation}
\label{sec:external_validation}

We tested whether the geospatial preprocessing and block-construction pipeline transfers to an independent county-scale cadastral dataset. The external case used Dongxing District, Neijiang, which was not used for model training, reward calibration, or Bishan policy evaluation. The Dongxing parcel layer contained 134{,}369 cadastral parcels in \texttt{EPSG:2359}; the same land-use mapping identified 45{,}735 farmland parcels, 30{,}643 forest parcels, and 57{,}991 other parcels.
...
\begin{table}[htbp]
\centering
\caption{External-county preprocessing validation on Dongxing District. The table reports data-ingestion and action-space construction results, not cross-county policy-performance results.}
...
The external case therefore passed the two data prerequisites for dynamic paired-slope-swap experiments: it contained mixed farmland-forest candidate blocks, and every generated block had continuous slope estimates for both farmland and forest components. This experiment strengthens the portability claim for the data pipeline and action representation. It does not, however, constitute a cross-county RL performance test, because Dongxing learned-environment training, local reward calibration, and learned-policy evaluation were not run.
...
Finally, we ran dynamic non-RL paired-swap baselines to test whether the Dongxing action space remained actionable after sequential state updates. Each episode used 100 decision steps and at most five positive-gap parcel pairs per selected block. The dynamic slope-gap strategy reduced area-weighted farmland slope by 1.218\%, and the dynamic area-weighted strategy reduced it by 0.883\%, compared with 0.358\% for the 200-seed random dynamic baseline (Table~\ref{tab:dongxing_dynamic}). Both deterministic dynamic baselines exceeded the random episodes for slope-change percentage, completed pair count, mean pair slope gap, and total pair slope gap, with one-sided empirical $p=0.004975$ using add-one correction for each metric. This result moves the external validation beyond a static opportunity screen, but it remains a slope-only, non-RL dynamic baseline rather than evidence of learned-policy transfer.

\begin{table}[htbp]
\centering
\scriptsize
\setlength{\tabcolsep}{2pt}
\caption{Dynamic non-RL paired-swap baselines on Dongxing District. Episodes use 100 decision steps and at most five positive-gap parcel pairs per selected block. Random values summarize 200 seeded dynamic episodes.}
\label{tab:dongxing_dynamic}
\begin{tabular}{lcccc}
\toprule
Baseline & Pairs & Slope (\%) & Mean gap & Total gap \\
\midrule
Slope-gap & 485 & $-1.218$ & 9.056 & 4{,}392.083 \\
Area-weighted gap & 487 & $-0.883$ & 7.538 & 3{,}671.162 \\
Random $\mu \pm \sigma$ & $426.545 \pm 13.649$ & $-0.358 \pm 0.087$ & $4.038 \pm 0.208$ & $1{,}722.523 \pm 109.533$ \\
\bottomrule
\end{tabular}
\end{table}
...
The calibration factor $\alpha = 0.185$ effectively shrinks the reward scale by $5.4\times$, which has a regularization effect similar to reward clipping or normalization. Its advantage over generic normalization is that the scaling is anchored to real trajectory evidence and then checked against downstream policy performance. The grid search experiment (Section~\ref{sec:grid_search}) confirms that $\alpha = 0.185$ falls within the optimal plateau $[0.15, 0.30]$, matching the grid-search optimum ($\alpha = 0.200$) to within 3\%.
...
\textbf{Policy-induced distribution shift.} The transition model is trained on random and greedy trajectories. The synchronized learned-vs-real diagnostic suggests that three trained calibrated policies remain close to the recorded global-state support and retain high action-mask agreement, but this is still a diagnostic over three checkpoints rather than an exhaustive coverage guarantee. If future policies visit states far from this distribution, prediction quality may degrade. Dyna-style hybrid training (alternating real and learned steps) could mitigate this but was not explored here.
...
\textbf{Reproducibility boundary.} The executable audit verifies that the recorded Bishan assets, trained transition model, policy-evaluation files, ablations, policy-induced diagnostics, and Dongxing feasibility outputs are mutually consistent. It is an audit of the stored data-to-result chain, not a fresh full retraining of all policies. A full computational reproduction would additionally rerun trajectory collection, transition training, 30 Bishan policy seeds, the 40-run reward-scaling grid, and the synchronized learned-vs-real diagnostic.

\textbf{Causal identification.} The calibration procedure uses observational trajectories rather than randomized treatment assignment. The sensitivity diagnostics show strong treatment-policy coupling in mixed trajectories and no control group in greedy-only trajectories. We therefore interpret the calibration as treatment-effect-informed reward regularization, not as definitive causal identification.

\textbf{Cross-county performance.} Policy-performance experiments remain limited to Bishan District. The Dongxing experiment shows that the cadastral preprocessing, DEM-based slope enrichment, block action-space construction, static slope-opportunity screening, and dynamic non-RL paired-swap baselines run on a second county-scale dataset. It does not yet test whether a Bishan-trained transition model or policy transfers to Dongxing. The Dongxing dynamic baselines are slope-only checks and do not include the full Bishan reward components for contiguity and \textit{baimu fang}. Cross-county performance claims require Dongxing trajectories, local reward calibration, and real-environment policy evaluation.

\textbf{DEM-derived slope uncertainty.} Dongxing slopes were derived from public DEM tiles with an approximately 76.44\,m cell size after reprojection. The DEM fully covers the parcel extent and produces complete parcel-level slope values, but small parcels may inherit representative-point values rather than cell-centre zonal means. The Dongxing results should therefore be read as an external feasibility and actionability check, not as a precision terrain-engineering assessment.
...
\begin{enumerate}
\item \textbf{Learned environments are viable for large-scale spatial planning when validated beyond aggregate similarity.} A 237K-parameter transition model trained on 12{,}000 trajectory samples achieves 0.9998 cosine similarity with real environment dynamics and supports effective policy training. Recorded-action and policy-induced diagnostics show high action-mask agreement but non-negligible global-state and reward errors, motivating final real-environment evaluation.
...
\item \textbf{External-county feasibility is supported, but policy transfer remains open.} On Dongxing District, the pipeline constructs 2{,}978 mixed farmland-forest blocks from 134{,}369 parcels, and dynamic non-RL paired-swap baselines reduce area-weighted farmland slope by 0.883--1.218\%. These results support data-pipeline and action-space portability, not cross-county learned-policy transfer.
...
\item \textbf{Policy-induced diagnostics support, but do not replace, real-environment evaluation.} Synchronized learned-vs-real rollouts for three calibrated policies retain 0.997 action-mask agreement, remain close to the trajectory support, and produce $-1.109\%$ mean real-environment slope change. These diagnostics address policy-induced distribution shift more directly than recorded-action rollouts, but final reported outcomes still come from the real environment.
...
\item \textbf{The verified claim boundary is narrower than a full cross-county RL claim.} The end-to-end audit supports the Bishan learned-policy chain and classifies Dongxing as external data/action-space/dynamic feasibility. A stronger external-validity claim would require Dongxing trajectories, local transition-model training or transfer testing, reward calibration, learned-policy optimization, and real-environment evaluation on Dongxing.
\end{enumerate}
